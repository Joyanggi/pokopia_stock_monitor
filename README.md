...
> 루리웹 핫딜 게시판에서 포켓몬 포코피아 관련 게시글을 주기적으로 찾아 VS Code 터미널과 데스크톱 알림으로 알려 주는 개인용 모니터링 도구

이 프로젝트는 공식 재고 API를 조회하는 서비스가 아닙니다. 빠르게 올라왔다가 종료되는 **커뮤니티 핫딜 게시글을 재입고 신호로 활용**해, 사람이 게시판을 계속 새로고침해야 하는 일을 자동화했습니다.

> **프로젝트 상태: 구매 성공 · 완료** — 실제 판매 글을 발견해 포코피아 구매에 성공했습니다. 목적 달성을 확인한 뒤 2026-07-14부로 운영과 기능 개발을 종료했습니다.
...
| 발견해도 늦게 보면 의미가 없었다 | VS Code 터미널에 제목·링크를 강조 출력하고 OS 알림과 소리도 지원 |
...
- 새 판매 글 발견 시 VS Code 터미널에 강조 로그와 게시글 링크 출력
- Windows 알림·시스템 소리 및 macOS 알림·효과음 지원
- 네트워크 오류 발생 시 프로세스를 종료하지 않고 다음 주기에 재시도

## 실제 사용 결과

별도의 웹사이트나 대시보드를 만들지는 않았습니다. VS Code에서 Python 프로그램을 실행해 주기별 확인 로그를 지켜보다가, 새 포코피아 판매 글의 제목과 링크가 터미널에 출력된 것을 확인했습니다. 해당 링크를 통해 실제 구매까지 성공하면서 프로젝트의 목적을 달성했습니다.

일회성 개인 문제에는 터미널 기반 실행만으로 충분했기 때문에, 구매 성공 이후 웹 UI나 서버 운영 기능을 추가하지 않고 프로젝트를 종료했습니다.
